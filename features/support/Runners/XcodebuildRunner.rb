module Runners
  ...
  class XcodebuildRunner < ComplexRunner
    def self.command(options)
        ...
        "xcodebuild -workspace AgileToolbox.xcworkspace -scheme #{options[:scheme]} -configuration Release -sdk iphonesimulator5.1 OBJROOT='#{ENV['HOME']}/UIAutomation/#{options[:scheme]}/build/obj' SYMROOT='#{ENV['HOME']}/UIAutomation/#{options[:scheme]}/build/bin' build"
    end
